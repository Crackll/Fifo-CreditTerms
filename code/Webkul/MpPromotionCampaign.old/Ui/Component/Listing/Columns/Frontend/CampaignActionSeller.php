...
class CampaignActionSeller extends Column
{
    /**
     * @var UrlInterface
     */
    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as &$item) {
                $campaignJoin = '0';
                $campaignJoin = $this->helper->campaignSellerStatus($item['campaign_id']);
                if ($campaignJoin != 0) {
                    $campaignStatus = $this->helper->campaignStatus($item['campaign_id']);
                    if ($campaignStatus['code'] == 2) {
                         ...
                         $item[$fieldName.'_campaignId'] = $item['campaign_id'];
