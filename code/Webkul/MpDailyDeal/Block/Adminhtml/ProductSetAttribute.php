...
class ProductSetAttribute extends \Magento\Backend\Block\Template
{
    ...
    public function getDealsDateTime()
    {
        ...
        $proType = $product->getTypeId() ? $product->getTypeId() : $proType;
        $dealDateTime = [
            'deal_from_date'=> $dealStatus ? $this->_localeDate->date($dateFrom)->format('m/d/Y H:i:s') :'',
            'deal_to_date'=> $dealStatus ? $this->_localeDate->date($dateTo)->format('m/d/Y H:i:s'):'',
