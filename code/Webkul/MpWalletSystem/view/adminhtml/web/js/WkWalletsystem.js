        ...
        $.widget(
            'mage.WkWalletsystem', {
                ...
                _create: function () {
                    ...
                    $('#html-body').delegate(
                        '.wk_addamount','click',function () {
                            $('#customerid').val($(this).attr('customer-id'));
                            var customername = $(this).attr('customer-name');
                            $(self.options.askdata).find('.modal-title').text('Adjust Amount to '+customername+"'s Wallet");
                    ...
                    $('.walletsystem-creditrules-edit #edit_form').on(
                        'submit', function () {
                    ...
                    $(".totals-actions").click(
                        function () {
                            setTimeout(
                                function () {
                                    $('.sales-order-creditmemo-new .order-totals-actions .actions button').each(
                                        function () {
                                            if (self.options.userHasWallet) {
                                                var refundOfflineButton = $(this);
                                                if (refundOfflineButton.length > 0 && self.options.invoiceId) {
                    ...
                    $('.sales-order-creditmemo-new .order-creditmemo-tables button').on(
                        'click', function () {
                            $('.sales-order-creditmemo-new .order-totals-actions .actions button').each(
                                function () {
                                    if (self.options.userHasWallet) {
