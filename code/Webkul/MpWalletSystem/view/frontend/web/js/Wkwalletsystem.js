        ...
        $.widget(
            'mage.Wkwalletsystem', {
                ...
                _create: function () {
                    ...
                    $('body #shipping-method-buttons-container button.continue').on(
                        'click', function (e) {
                    ...
                    $("body").delegate(
                        'a.action.edit, a.action.back', "click", function (e) {
                            e.preventDefault();
                    ...
                    $('.wk_ws_add_payment').click(
                        function () {
                            if ($('#transfer-form-data').valid()!== false) {
                                $('body').trigger('processStart');
                            }
                        }
                    )

                    $(self.options.deletelink).on(
                        'click', function (e) {
                            var element = $(this);
                                ...
                                var dicision = confirm(
                                    {
                                        ...
                                        actions: {
                                            confirm: function () {
                                                ...
                                                $.each(
                                                    deleteurl.data, function (key, value) {
                    ...
                    $('.wk_ws_sub_head_transfer').on(
                        'click', function () {
                            self.addTransferBlockToPage();
                        }
                    );
                    $('.wk_ws_sub_add_payee').on(
                        'click', function () {
                ...
                setwalletamount :function () {
                    ...
                    var restamount = 0;
                    var type = 'reset';
                    var ajaxUrl = window.authenticationPopup.baseUrl+'mpwalletsystem/index/applypaymentamount';
                    $('body').trigger('processStart');
                    var ajaxreturn = $.ajax(
                        {
                            url:ajaxUrl,
                    ...
                    $('.wk_ws_bank_transfer').modal(
                        {
                            ...
                            innerScroll: true,
                            validation:{},
                            title: "Enter Transfer Details", //write your popup title
                            buttons: [
                            {
                                text: $.mage.__('Submit'),
                                class: 'button',
                                click: function () {
                                    var form = $('#mpwalletsystem_trasfer_amount');
                ...
                addPayeeBlockOnPage: function () {
                    ...
                    $('.wk_ws_add_payee_modal').modal(
                        {
                            ...
                            innerScroll: true,
                            validation:{},
                            title: "Enter Payee Details", //write your popup title
                            buttons: [
                            {
                                text: $.mage.__('Submit'),
                                class: 'button',
                                click: function () {
                                    var form = $('#mpwalletsystem_add_payee');
                                    if ($(form).validation() && $(form).validation('isValid')) {
                    ...
                    $('.wk_ws_add_payee_modal').modal().on(
