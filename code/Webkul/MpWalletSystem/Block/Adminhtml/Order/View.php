...
namespace Webkul\MpWalletSystem\Block\Adminhtml\Order;
...
class View extends \Magento\Sales\Block\Adminhtml\Order\View
{
    ...
    protected function _construct()
    {
        ...
        $orderId = $data = $this->getRequest()->getParam('order_id');
        if ($order->getStatus() !=__('complete')) {
            $orderItems = $order->getAllVisibleItems();
            $flag = false;
            $label = "Invoice";
            $location = $this->getInvoiceUrl();
            foreach ($orderItems as $item) {
                if ($item->getSku() == "wk_wallet_amount") {
                    $flag = true;
                    $label = __("Confirm Wallet Order");
                    $location = $this->getUrl('mpwalletsystem/wallet/acceptorder/');
                }
            }
            if ($flag) {
