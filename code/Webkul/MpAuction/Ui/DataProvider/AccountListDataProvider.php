...
class AccountListDataProvider extends \Magento\Ui\DataProvider\AbstractDataProvider
{
    ...
    */
    public function __construct(
        ...
        $collectionData =  $auctionProduct->create()->getCollection()
                ->addFieldToFilter('customer_id', $auctionHelperData->getCurrentCustomerId())
        ...
        if (isset($getParamFilters['start_auction_time'])) {
            ...
            ?$getParamFilters['start_auction_time']['from']." 00:00:00":date("Y-d-m")." 00:00:00";

            $startAuctionToTime = isset($getParamFilters['start_auction_time']['to'])
            ?$getParamFilters['start_auction_time']['to']." 23:59:59":date('Y-d-m')." 23:59:59";
        ...
        if (isset($getParamFilters['stop_auction_time'])) {
            ...
            ?$getParamFilters['stop_auction_time']['from']:date('Y-d-m');

            $stopAuctionToTime = isset($getParamFilters['stop_auction_time']['to'])
            ?$getParamFilters['stop_auction_time']['to']:date('Y-d-m');
