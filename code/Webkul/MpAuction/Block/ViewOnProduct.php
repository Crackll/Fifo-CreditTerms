...
class ViewOnProduct extends \Magento\Framework\View\Element\Template
{
    ...
    public function getAuctionDetail()
    {
        ...
        if ($auctionConfig['enable']) {
            ...
            $aucAutoData = $this->autoAuctionFactory->create()->getCollection()
        ...
        $auctionData['enable']=$this->moduleManager->isOutputEnabled('Webkul_Xtremo');
        return $auctionData;
    }
    ...
    public function getAuctionDetailAftetEnd($auctionData)
    {
        ...
        $currentCustomerId = $this->helperData->getCurrentCustomerId() || $this->httpContext->getValue('customer_id');
        if ($currentCustomerId) {
            ...
            if (in_array($currentCustomerId, $custList)) {
                ...
                && $this->getAuctionConfiguration()['enable_auto_outbid_msg']) {
