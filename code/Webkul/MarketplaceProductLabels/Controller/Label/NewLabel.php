...
class NewLabel extends Action
{
    ...
    public function execute()
    {
        ...
        if ($isPartner == 1 && $this->labelHelperData->getConfigData('label_manage')) {
            if ($this->mpHelperData->getIsSeparatePanel()) {
                $resultPage->addHandle('mplabels_layout2_label_newlabel');
                $resultPage->getConfig()->getTitle()->set(__('Add New Product Label'));
                return $resultPage;
            } else {
                $resultPage->getConfig()->getTitle()->set(__('Add New Product Label'));
                return $resultPage;
