...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    const SELLER_STATUS_PACKAGE_NOT_RECEIVED = 0;
    const SELLER_STATUS_PACKAGE_RECEIVED = 1;
    const SELLER_STATUS_PACKAGE_DISPATCHED = 2;
    const SELLER_STATUS_SOLVED = 2;
    const SELLER_STATUS_DECLINED = 3;
    const SELLER_STATUS_ITEM_CANCELED = 4;
     ...
     * @param InvoiceItemCollection $shipmentItemCollection
    ...
    public function processCancellation($rmaId)
    {
        ...
        foreach ($productDetails as $item) {
            $qty = $item->getQty();
            $totalCanceledQty = $qty + $item->getQtyCanceled();
            if ($item->getQtyOrdered() == $totalCanceledQty) {
                $this->setOrderStatus($item->getOrderId(), $rmaId);
                $this->cancelOrder($item->getOrderId());
            }
    ...
    /**
     * If MpAssign Module Enable Then
     * get sellerId of the product
     *
     * @param int $orderId
     * @return int
     */
